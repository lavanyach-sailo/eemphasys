...
//		int cell = Integer.parseInt(cellValue);
	...
	public static TestEnsureRequest scrollTo(JsonNode navActionParams, String file, String testCaseStepId)
			throws Throwable {
		TestEnsureRequest testEnsureRequest = null;
		testEnsureRequest = processNavActionNode(navActionParams, ProcessConstants.SCROLL, file, testCaseStepId);
		return testEnsureRequest;
	}
	...
	public static TestEnsureRequest processNavActionNode(JsonNode navActionParams, String processType, String file,
		...
		try {
			...
			case ProcessConstants.GETTEXT:
				...
				log.info("variables" + variables.toString());
				break;
			case ProcessConstants.SCROLL:
				Actions action = new Actions(driver);
				WebElement element = driver.findElement(By.xpath(finalXpath));
				// action.moveToElement(element);
				TestBase.scrollTo(finalXpath, ignore);
				log.info("Scroll WebElement-->" + element);
				// ((JavascriptExecutor)driver).executeScript("arguments[0].scrollIntoView(false);",
				// element);

				// WebElement we =
				// driver.findElement(By.xpath(finalXpath)).getShadowRoot().findElement(By.cssSelector("div#icon"));
				// log.info("CLICKSHADOWELEMENT"+we);
				break;
